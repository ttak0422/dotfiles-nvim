...
local behaviour = {auto_suggestions = true, auto_set_highlight_group = true, auto_set_keymaps = true, support_paste_from_clipboard = true, minimize_diff = true, enable_token_counting = true, auto_approve_tool_permissions = true, auto_check_diagnostics = true, auto_apply_diff_after_generation = false, auto_focus_on_diff_view = false, auto_focus_sidebar = false, auto_suggestions_respect_ignore = false, enable_fastapply = false, include_generated_by_commit_line = false, use_cwd_as_project_root = false}
local mappings
do
  local diff = {ours = "co", theirs = "ct", all_theirs = "ca", both = "cb", cursor = "cc", next = "]x", prev = "[x"}
  local suggestion = {accept = "<C-x><C-a>", next = "<M-]>", prev = "<M-[>", dismiss = "<M-e>"}
...
local function _5_(_, _0, cb)
  if cb then
    return cb("Please commit the staged changes only, based on the current Git status.")
  else
    return nil
  end
end
