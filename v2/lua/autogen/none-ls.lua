...
sources = {diagnostics.actionlint, diagnostics.checkmake, diagnostics.checkstyle.with({runtime_condition = _1_, extra_args = {"-c", (vim.g.checkstyle or "/google_checks.xml")}}), diagnostics.deadnix, diagnostics.dotenv_linter, diagnostics.editorconfig_checker, diagnostics.gitlint, diagnostics.ktlint, diagnostics.selene, diagnostics.staticcheck, diagnostics.statix, diagnostics.stylelint, diagnostics.vint, diagnostics.yamllint, formatting.prettier.with({prefer_local = "node_modules/.bin", runtime_condition = _2_, filetypes = {"javascript", "javascriptreact", "typescript", "typescriptreact", "vue", "css", "scss", "less", "json", "jsonc", "markdown", "markdown.mdx", "graphql", "handlebars", "svelte", "astro", "htmlangular"}}), require("none-ls.diagnostics.eslint"), formatting.biome, formatting.fantomas, formatting.fnlfmt, formatting.gofumpt, formatting.goimports, helpers.make_builtin({name = "idea", method = FORMATTING, filetypes = {"java", "groovy", "kotlin"}, runtime_condition = _3_, generator_opts = {command = args.idea, args = _4_, timeout = 20000, from_temp_file = true, to_temp_file = true, to_stdin = false}, factory = helpers.formatter_factory}), formatting.google_java_format.with({runtime_condition = _5_, timeout = 20000}), formatting.ktlint, formatting.nixfmt, formatting.shfmt, formatting.stylelint, formatting.stylua, formatting.tidy, formatting.yamlfmt, formatting.yapf}
...
vim.api.nvim_create_user_command("NoneLsInfo", _6_, {})
local function _7_()
  return vim.cmd("NullLsLog")
end
return vim.api.nvim_create_user_command("NoneLsLog", _7_, {})
