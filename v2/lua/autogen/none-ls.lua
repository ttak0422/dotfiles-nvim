...
local function _4_(params)
  local bufname = vim.api.nvim_buf_get_name(params.bufnr)
  local targets = {".eslintrc", "eslint.config.js"}
  return vim.fs.root(bufname, targets)
end
local function _5_()
...
sources = {diagnostics.actionlint, diagnostics.checkmake, diagnostics.checkstyle.with({runtime_condition = _1_, extra_args = {"-c", (vim.g.checkstyle or "/google_checks.xml")}}), diagnostics.deadnix, diagnostics.dotenv_linter, diagnostics.editorconfig_checker.with({runtime_condition = _2_}), diagnostics.gitlint, diagnostics.ktlint, diagnostics.mypy, diagnostics.selene, diagnostics.semgrep, diagnostics.sqruff, diagnostics.staticcheck, diagnostics.statix, diagnostics.stylelint, diagnostics.terraform_validate, diagnostics.vint, diagnostics.yamllint, formatting.prettier.with({prefer_local = "node_modules/.bin", runtime_condition = _3_, filetypes = {"javascript", "javascriptreact", "typescript", "typescriptreact", "vue", "css", "scss", "less", "graphql", "handlebars", "svelte", "astro", "htmlangular"}}), diagnostics.eslint.with({runtime_condition = _4_}), formatting.biome, formatting.fantomas, formatting.fnlfmt, formatting.gofumpt, formatting.goimports, helpers.make_builtin({name = "idea", method = FORMATTING, filetypes = {"java", "groovy", "kotlin"}, runtime_condition = _5_, generator_opts = {command = args.idea, args = _6_, timeout = 20000, from_temp_file = true, to_temp_file = true, to_stdin = false}, factory = helpers.formatter_factory}), formatting.google_java_format.with({runtime_condition = _7_, timeout = 20000}), formatting.ktlint, formatting.nixfmt, formatting.shfmt, formatting.sqruff, formatting.stylelint, formatting.stylua, formatting.terraform_fmt, formatting.tidy, formatting.yamlfmt, formatting.yapf}
...
vim.api.nvim_create_user_command("NoneLsInfo", _8_, {})
local function _9_()
  return vim.cmd("NullLsLog")
end
return vim.api.nvim_create_user_command("NoneLsLog", _9_, {})
