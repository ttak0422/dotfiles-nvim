...
do
  ...
  local function _11_()
    local case_12_ = vim.fn.getcmdtype()
    if (case_12_ == "/") then
      return search_src
    elseif (case_12_ == "?") then
      return search_src
    elseif (case_12_ == ":") then
      return cmd_src
    elseif (case_12_ == "@") then
  ...
  _10_ = {enabled = true, completion = {list = {selection = {auto_insert = false, preselect = false}}}, keymap = {preset = "none", ["<Tab>"] = {"show", "select_next", "fallback"}, ["<S-Tab>"] = {"show", "select_prev", "fallback"}, ["<C-space>"] = {"show", "fallback"}, ["<C-n>"] = {"show", "select_next", "fallback"}, ["<C-p>"] = {"show", "select_prev", "fallback"}, ["<Right>"] = {"select_next", "fallback"}, ["<Left>"] = {"select_prev", "fallback"}, ["<C-y>"] = {"select_and_accept", "fallback"}, ["<C-e>"] = {"cancel", "fallback_to_mappings"}, ["<End>"] = {"hide", "fallback"}}, sources = _11_}
end
