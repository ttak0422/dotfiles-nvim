...
operators = {
  b = {
    next = { opts = { desc = "> buffer" }, rhs = "<Cmd>bnext<CR>" },
    prev = { opts = { desc = "< buffer" }, rhs = "<Cmd>bprevious<CR>" },
  },
  B = {
    next = { opts = { desc = "> buffer (history)" }, rhs = "<Cmd>BufSurfForward<CR>" },
    prev = { opts = { desc = "< buffer (history)" }, rhs = "<Cmd>BufSurfBack<CR>" },
  },
  d = { next = { opts = { desc = "> warning" }, rhs = _3_ }, prev = { opts = { desc = "< warning" }, rhs = _4_ } },
  D = { next = { opts = { desc = "> error" }, rhs = _5_ }, prev = { opts = { desc = "< error" }, rhs = _6_ } },
  e = { next = { opts = { desc = "> edit" }, rhs = "g;" }, prev = { opts = { desc = "< edit" }, rhs = "g," } },
  j = { next = { opts = { desc = "> jump" }, rhs = "<C-i>" }, prev = { opts = { desc = "< jump" }, rhs = "<C-o>" } },
  h = { next = { opts = { desc = "> harpoon" }, rhs = _7_ }, prev = { opts = { desc = "< harpoon" }, rhs = _8_ } },
  q = {
    next = { opts = { desc = "> qf (item)" }, rhs = "<Cmd>Qnext<CR>" },
    prev = { opts = { desc = "< qf (item)" }, rhs = "<Cmd>Qprev<CR>" },
  },
  ["<C-q>"] = {
    next = { opts = { desc = "> qf (file)" }, rhs = "<Cmd>cnfile<CR>" },
    prev = { opts = { desc = "< qf (file)" }, rhs = "<Cmd>cpfile<CR>" },
  },
  ["<M-q>"] = {
    next = { opts = { desc = "> qf (list)" }, rhs = safe_qf_cnewer },
    prev = { opts = { desc = "< qf (list)" }, rhs = safe_qf_colder },
  },
  g = nap.gitsigns(),
}
return nap.setup({
