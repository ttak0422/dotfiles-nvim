  ...
  do
    ...
    if (ok and (chan > 0)) then
      for _, f in ipairs(vim.fn.argv()) do
        local rok, _0 = pcall(vim.rpcrequest, chan, "nvim_cmd", {cmd = "edit", args = {vim.fn.fnamemodify(f, ":p")}}, {})
        if rok then
          sent = true
        else
        end
...
do
  ...
  local desc
  local function _8_(d)
    return {noremap = true, silent = true, desc = d}
  end
  desc = _8_
  local lua_
  local function _9_(mod, f, opt)
    local function _10_()
      if (opt == nil) then
