...
local file = {
	...
	{ provider = " " },
	{
		...
		{
			provider = function(self)
				local patterns = self.maven[self.filetype]
				if patterns then
					for _, pattern in ipairs(patterns) do
						if vim.startswith(self.path, pattern) then
							return self.path:sub(#pattern + 1)
						end
					end
				end
				return self.path
		...
		},
		{
			...
			provider = function(self)
				...
				if patterns then
					for _, pattern in ipairs(patterns) do
						if vim.startswith(self.path, pattern) then
							return vim.fn.pathshorten(self.path:sub(#pattern + 1))
						end
					end
				end
				return vim.fn.pathshorten(self.path)
			end,
		},
		{ provider = "" },
	},
	...
	opts = {
		...
		disable_winbar_cb = function(args)
			return conditions.buffer_matches({
				...
				filetype = { "^git.*", "Trouble", "skk-terminal-input" },
			}, args.buf)
		end,
	},
})
