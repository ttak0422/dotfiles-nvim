-- [nfnl] Compiled from fnl/heirline.fnl by https://github.com/Olical/nfnl, do not edit.
local heirline = require("heirline")
local conditions = require("heirline.conditions")
local utils = require("heirline.utils")
local hydra = require("hydra.statusline")
local colors = require("morimo.colors")
local colors0 = {fg = colors.fg, bg = colors.bg0, red = colors.red, green = colors.green, blue = colors.blue, grey = colors.grey0, orange = colors.orange, purple = colors.purple, cyan = colors.cyan, git_add = colors.lightGreen, git_change = colors.lightBlue, git_del = colors.lightRed}
local opts = {colors = colors0}
...
do
  ...
  readonly_symbol = {condition = _1_, provider = icons.lock, hl = {fg = colors0.bg}}
  local vim_symbol = {provider = icons.vim, hl = {fg = colors0.bg}}
...
do
  ...
  active = {{provider = _4_}, {{provider = _5_, hl = {fg = colors0.git_add}}, {provider = _6_, hl = {fg = colors0.git_change}}, {provider = _7_, hl = {fg = colors0.git_del}}}, condition = conditions.is_git_repo, init = _8_}
...
do
  ...
  root = {init = _18_, provider = _19_, update = {"DirChanged"}, hl = {fg = colors0.bg, bg = colors0.orange}, static = {alias = {[""] = "ROOT"}}}
end
local help_name
local function _20_()
...
help_name = {condition = _20_, provider = _21_, hl = {fg = colors0.fg}}
local terminal_name
local function _22_()
  local name, _ = vim.api.nvim_buf_get_name(0):gsub(".*:", "")
  return name
end
terminal_name = {provider = _22_, hl = {fg = colors0.fg}}
local tabline
do
  ...
  do
    ...
    local function _46_(self)
      if self.is_active then
        return colors0.orange
      else
        return colors0.bg
      end
    end
    buffer_block = utils.surround({icons.fill, icons.fill}, _46_, {file_block})
    buffer_line = utils.make_buflist(buffer_block, {provider = "<", hl = {fg = colors0.grey}}, {provider = ">", hl = {fg = colors0.grey}})
  end
  local tabpage
...
special_status = {mode, align, help_name, align, {provider = _52_, hl = {fg = colors0.bg, bg = colors0.blue}, update = {"WinNew", "WinClosed", "BufEnter"}}, condition = _53_}
...
terminal_status = {mode, align, terminal_name, align, {provider = _54_, hl = {fg = colors0.bg, bg = colors0.red}, update = {"WinNew", "WinClosed", "BufEnter"}}, condition = _55_}
...
local statusline = {special_status, terminal_status, default_status_line, hl = {fg = colors0.fg, bg = colors0.bg, bold = true}, fallthrough = false}
vim.o.showtabline = 2
vim.o.laststatus = 3
