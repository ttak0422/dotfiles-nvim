...
local function _2_(diagnostic)
  diagnostic.severity = vim.diagnostic.severity.Warning
  return nil
end
local function _3_(...)
  return not is_active_lsp("denols")
end
sources = {diagnostics.staticcheck.with({diagnostics_postprocess = _2_}), formatting.stylua, formatting.shfmt, formatting.google_java_format, formatting.yapf, formatting.prettier.with({prefer_local = "node_modules/.bin", runtime_condition = _3_}), formatting.tidy, formatting.gofumpt, formatting.fnlfmt, formatting.nixfmt}
